...
class ResourceTests(TestCase):
    ...
    def test_external_resource(self):
        resource = external_resource('wordlist.txt', 'tests.resources')
        self.assertTrue(resource.endswith(os.path.join('tests', 'resources', 'wordlist.txt')))
        self.assertTrue(os.path.lexists(resource))

    def test_icon_resource(self):
        icon = icon_resource('0.png', 'tests.resources.images')
        self.assertTrue(icon.startswith('file:///') and icon.endswith('tests/resources/images/0.png'))
    ...
    def test_image_resources(self):
        ...
        for i, j in enumerate(sorted(images)):
            self.assertTrue(j.endswith(os.path.join('tests', 'resources', 'images', '%d.png' % i)))
